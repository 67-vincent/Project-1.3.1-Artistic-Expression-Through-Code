...
message_font = ("Arial", 14, "bold") 

#-----initialize turtles-----

#Background Heart Stamper leaves the hearts on screen
stamper = trtl.Turtle()
stamper.hideturtle() 
...
# 6. UI Turtles
#ui that writes the score
score_writer = trtl.Turtle()
score_writer.hideturtle()
score_writer.penup()
score_writer.goto(115, 215)
# ui that writes the timer
counter = trtl.Turtle()
counter.hideturtle()
counter.penup()
counter.goto(-170, 215)

# ui that writes the name of the turtle game
...
#-----functions--------
# draws the outline design things
def draw_design():
    border.penup()
    border.goto(-180, 250)
    border.pendown()
    for _ in range(2):
        ...
        for _ in range(16):
            border.forward(15); border.penup(); border.forward(15); border.pendown()
        border.right(90)
# function writing the game name on the bottom
def write_bottom_message():
    text_writer.clear()
    text_writer.write("Panda Game", align="center", font=message_font)
# draws the balloon string and moves it to new x coords
def draw_string():
    string_drawer.clear()
    if not timer_up:
        string_drawer.penup()
...
# stamper goes to x coord, stamps, moves to new x coord and updates hwen clicked
def move_balloon():
    if not timer_up:
...
# leaderboard, calls to the leaderboard.py file
